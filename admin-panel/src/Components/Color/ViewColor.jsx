...
export default function ViewColor() {
  ...
  const changeStatus = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Color_API}/change-status`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Color_API}/delete`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
                ...
                <tr className="text-md">
                  <th className="px-4 py-2 w-[2%]">
                    <input
                      ...
                      checked={colors.length > 0 && checkBoxValue.length === colors.length}
                      className="w-4 h-4"
                    />
                  </th>

                  <th className="px-4 py-2">COLOR NAME</th>
                  <th className="text-center px-4 py-2 w-[12%]">COLOR</th>
                  <th className="text-center px-4 py-2 w-[12%]">CODE</th>
                      ...
                      return (
                        <tr key={i} className="border-t hover:bg-gray-50 transition-all">
                          ...
                          <td className="text-center px-4 py-3">
                            <div className="w-10 h-10 m-auto border border-gray-400 rounded-md " style={{background : v.code}}></div>
                          </td>
                          <td className="text-center px-4 py-3">{v.code}</td>
