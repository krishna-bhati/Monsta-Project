...
export default function ViewCountry() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Country_API}/view`, {
      page: currentPage,
      name: searchName,
    })
      .then((result) => {
        if (result.data._status == true) {
          setCountries(result.data._data)
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValue.length == countries.length) {
      setCheckBoxValue([]);
      setButtonDisable(true);
    } else {
  ...
  const changeStatus = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Country_API}/change-status`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Country_API}/delete`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
                ...
                <tr className="text-md">
                  <th className="px-4 py-2 w-[2%]">
                    <input
                      ...
                      checked={checkBoxValue.length === countries.length}
                      className="w-4 h-4"
                    />
                  </th>
