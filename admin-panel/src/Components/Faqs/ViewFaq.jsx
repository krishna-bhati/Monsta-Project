...
export default function ViewFaq() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_FAQ_API}/view`, {
      page: currentPage,
  ...
  }, [currentPage, searchQuestion, apiStatus])

  const searchHandler = (event) => {
    event.preventDefault();
    setCurrentPage(1);
    setSearchQuestion(event.target.name.value);
  }
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValue.length == faqs.length) {
      setCheckBoxValue([]);
      setButtonDisable(true);
    } else {
  ...
  const changeStatus = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_FAQ_API}/change-status`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_FAQ_API}/delete`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
                ...
                <tr className="text-md">
                  <th className="px-4 py-2 w-[2%]">
                    <input
                      ...
                      checked={faqs.length > 0 && checkBoxValue.length === faqs.length}
                      className="w-4 h-4"
                    />
                  </th>

                  <th className="px-4 py-2">Question</th>
                  <th className="px-4 py-2 w-[32%]">Answer</th>
                    ...
                    faqs.map((v, i) => {
                      return (
                        <tr key={i} className="border-t hover:bg-gray-50 transition-all">
                          ...
                          <td className="px-4 py-3 font-medium text-gray-900 text-base">{v.question}</td>
                          <td className="px-4 py-3 font-medium text-gray-900 text-base text-justify">{v.answer}</td>
