...
export default function ViewMaterial() {
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValue.length == materials.length) {
      setCheckBoxValue([]);
      setButtonDisable(true);
    } else {
  ...
  const changeStatus = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Material_API}/change-status`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Material_API}/delete`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
                ...
                <tr className="text-md">
                  <th className="px-4 py-2 w-[2%]">
                    <input
                      ...
                      checked={materials.length > 0 && checkBoxValue.length === materials.length}
                      className="w-4 h-4"
                    />
                  </th>
