...
export default function AddCategory() {
  ...
  const [parentCategoriesDetail, setParentCategoriesDetails] = useState('')
  ...
  const formHandler = (event) => {
    ...
    if (!updatedId) {
      ...
      axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_Parent_Category_API}/update/${updatedId}`, event.target)
        .then((result) => {
          if (result.data._status == true) {
            toast.success(result.data._message)
            event.target.reset();
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <form onSubmit={formHandler} autoComplete="off" className="border border-t-0 p-3 rounded-b-md border-slate-400">
            <div className="flex gap-5">
              <div className="w-1/3 ">
                <label className=" block text-md mb-2 font-medium">Category Image</label>
                <input type="file"
                  name="image"
                  accept="image/*"
                  id="image"
              ...
              <div className="w-2/3">
                <div className="mb-5">
                  <label className=" block text-md font-medium">Category Name</label>
                  <input type="text" defaultValue={parentCategoriesDetail.name} name="name" className="text-md border-2 outline-0 border-gray-300 shadow-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full py-1.5 px-1.5 mt-1"
                    placeholder="Enter Category Name" />
                </div>

                <div className="mb-5">
                  <label className=" block text-md font-medium">Order</label>
                  <input type="number" defaultValue={parentCategoriesDetail.order} name="order" className="text-md border-2 outline-0 border-gray-300 shadow-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full py-1.5 px-1.5 mt-1"
