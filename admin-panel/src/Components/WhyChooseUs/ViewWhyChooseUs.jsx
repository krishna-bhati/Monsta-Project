...
export default function ViewWhyChooseUs() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_WHY_CHOOSE_US_API}/view`, {
      page: currentPage,
      name: searchName,
    })
      .then((result) => {
        if (result.data._status == true) {
          setWhyChooseUs(result.data._data)
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValue.length == whyChooseUs.length) {
      setCheckBoxValue([]);
      setButtonDisable(true);
    } else {
  ...
  const changeStatus = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_WHY_CHOOSE_US_API}/change-status`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.patch(`${import.meta.env.VITE_Base_Url}/${import.meta.env.VITE_WHY_CHOOSE_US_API}/delete`, {
      ids: checkBoxValue,
    })
      .then((result) => {
        if (result.data._status == true) {
                ...
                <tr className="text-md">
                  <th className="px-4 py-2 w-[2%]">
                    <input
                      ...
                      checked={whyChooseUs.length > 0 && checkBoxValue.length === whyChooseUs.length}
                      className="w-4 h-4"
                    />
                  </th>

                  <th className="px-4 py-2">Name</th>
                  <th className="text-center px-4 py-2 w-[25%]">Description</th>
                    ...
                    whyChooseUs.map((v, i) => {
                      return (
                        <tr key={i} className="border-t hover:bg-gray-50 transition-all">
                          ...
                          <td className="px-4 py-3 text-center">{v.description}</td>
