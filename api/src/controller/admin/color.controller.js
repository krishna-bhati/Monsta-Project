...
exports.create = (request, response) => {
    ...
    try {
        var saveData = new colorModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        await colorModel.find(filter).select('name code status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await colorModel.updateOne(
            {
