...
exports.create = (request, response) => {
    ...
    try {
        var saveData = new defaultModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        await defaultModel.find(filter).select('name status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await defaultModel.updateOne(
            {
...
exports.destroy = async (request, response) => {
    try {
        ...
        var saveData = await defaultModel.updateMany({
