...
exports.create = (request, response) => {
    ...
    try {
        var saveData = new faqModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        if (request.body) {
            if (request.body.question != undefined && request.body.question != '') {
        ...
        total_records = await faqModel.find(filter).countDocuments();

        await faqModel.find(filter).select('question answer status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.details = async (request, response) => {
    try {
        ...
        await faqModel.findById(request.params.id)
            .then((result) => {
                if (result) {
                    const data = {
                        _status: true,
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await faqModel.updateOne(
            {
...
exports.destroy = async (request, response) => {
    try {
        ...
        var saveData = await faqModel.updateMany({
