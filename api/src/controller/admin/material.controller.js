...
exports.create = (request, response) => {
    ...
    try {
        var saveData = new materialModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_records = await materialModel.find(filter).countDocuments();

        await materialModel.find(filter).select('name status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.details = async (request, response) => {
    try {
        ...
        await materialModel.findById(request.params.id)
            .then((result) => {
                if (result) {
                    const data = {
                        _status: true,
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await materialModel.updateOne(
            {
