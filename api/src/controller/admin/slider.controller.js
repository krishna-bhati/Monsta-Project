...
exports.create = async (request, response) => {
    ...
    try {
        var saveData = new sliderModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_records = await sliderModel.find(filter).countDocuments();

        await sliderModel.find(filter).select('name image status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
                    ...
                    const data = {
                        ...
                        _paginate: paginate,
                        _image_path: process.env.sliders_image,
                        _data: result,
...
exports.details = async (request, response) => {
    try {
        ...
        await sliderModel.findById(request.params.id)
            .then((result) => {
                if (result) {
                    const data = {
                        _status: true,
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await sliderModel.updateOne(
            {
...
exports.destroy = async (request, response) => {
    try {
        ...
        var saveData = await sliderModel.updateMany({
