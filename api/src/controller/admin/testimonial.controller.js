...
exports.create = (request, response) => {
    ...
    try {
        var saveData = new testimonialModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        await testimonialModel.find(filter).select('name designation image rating status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await testimonialModel.updateOne(
            {
