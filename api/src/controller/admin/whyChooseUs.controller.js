const whyChooseUsModel = require('../../models/whyChooseUs.model');
...
exports.create = async (request, response) => {
    ...
    try {
        var saveData = new whyChooseUsModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_records = await whyChooseUsModel.find(filter).countDocuments();

        await whyChooseUsModel.find(filter).select('name description image status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
                    ...
                    const data = {
                        ...
                        _paginate: paginate,
                        _image_path: process.env.whyChooseUs_image,
                        _data: result,
...
exports.details = async (request, response) => {
    try {
        ...
        await whyChooseUsModel.findById(request.params.id)
            .then((result) => {
                if (result) {
                    const data = {
                        _status: true,
...
exports.update = async (request, response) => {
    try {
        ...
        var saveData = await whyChooseUsModel.updateOne(
            {
...
exports.destroy = async (request, response) => {
    try {
        ...
        var saveData = await whyChooseUsModel.updateMany({
